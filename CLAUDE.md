...
- `src/simulation.jl` - Bayesian strength estimation (`estimate_strengths`): uninformative prior with PCS as observation, trajectory signal, season-adaptive VG variance, class-aware PCS blending for stage races, domestique strength discount. `BayesianConfig` uses 3 precision scale factors (market, history, ability) with fixed within-group ratios; accessor functions compute effective variances. Block-correlation discount groups signals into the same 3 clusters. Also retains `predict_expected_points` (MC simulation) for backtesting compatibility.
...
- `src/report_helpers.jl` - HTML page generation (`html_page`, `html_table`, `html_callout`, `html_heading`), Plotly chart helpers, SVG chart functions (PIT histograms, box plots, team totals), signal waterfall, VG simulation draws
