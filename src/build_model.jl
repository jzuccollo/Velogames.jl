...
function build_model_stage(
    ...
    JuMP.@constraint(model, sum(x) == n) # exactly n riders must be chosen
    JuMP.@constraint(model, df[!, :allrounder]' * x >= STAGE_CLASS_MINIMUMS[:allrounder])
    JuMP.@constraint(model, df[!, :sprinter]' * x >= STAGE_CLASS_MINIMUMS[:sprinter])
    JuMP.@constraint(model, df[!, :climber]' * x >= STAGE_CLASS_MINIMUMS[:climber])
    JuMP.@constraint(model, df[!, :unclassed]' * x >= STAGE_CLASS_MINIMUMS[:unclassed])
    JuMP.optimize!(model)
    if termination_status(model) != OPTIMAL
        @warn("The model was not solved correctly.")
        return
