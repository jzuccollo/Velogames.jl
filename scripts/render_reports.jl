...
<style>
  ...
  .info .tip {
    ...
    width: max-content; max-width: 15em;
    background: var(--navy); color: #fff; padding: 0.55em 0.75em; border-radius: 7px;
    font: 400 0.8rem/1.45 var(--font-body); text-transform: none; letter-spacing: 0;
...
(function () {
  ...
  function discStrip(label, arr, avgLabel) {
    ...
    // Credits committed: a rider's price is paid once per game, not per race. The classics
    // game charges one season-long price (so all of a year's one-day races share it), while
    // each grand tour is its own game. Count each distinct game's cost once.
    var seen = {}, credits = 0;
    arr.forEach(function (r) {
      var gk = r.type === "tour" ? (r.year + "|" + r.event) : ("oneday|" + r.year);
      if (!seen[gk]) { seen[gk] = true; credits += (r.cost || 0); }
    });
    var ppc = credits ? (tot / credits) : 0;
    var isTour = arr[0].type === "tour";
    var noun = isTour ? "grand tour" : "classic";
    var infoTot = "Total Velogames points across every " + noun + " we have data for — all archived seasons combined, not a single year.";
    var infoAvg = "Total points ÷ the number of " + (isTour ? "grand tours" : "classics") + " ridden (across all archived seasons).";
    var infoPpc = isTour
      ? "Total points ÷ credits, counting each grand tour’s price once (a grand tour is one game)."
      : "Total points ÷ credits. The classics price buys the rider for the whole season, so it’s counted once per season — not once per race.";
    var infoBest = "The most Velogames points scored in a single " + noun + ".";
    return '<div class="disc"><div class="disc-label">' + esc(label) + " &middot; " + arr.length +
      (arr.length === 1 ? " race" : " races") + "</div>" +
      '<div class="statstrip">' +
        stat(fmt(tot), "Velogames pts", infoTot) +
        stat(fmt(Math.round(tot / arr.length)), avgLabel, infoAvg) +
        stat(ppc.toFixed(1), "Points / credit", infoPpc) +
        stat(fmt(best), "Best haul", infoBest) +
      "</div></div>";
  }
})();
</script>
