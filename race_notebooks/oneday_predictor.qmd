...
racestartlistquality = "https://www.procyclingstats.com/race/world-championship/2025/startlist/startlist-quality"
racehash = "#WorldChamps"  # Leave empty if not available
...
excluded_riders = String["Quinten Hermans"]
...
```{julia}
#| echo: false
println("🎯 BUILDING OPTIMAL ONE-DAY TEAM")
println("="^40)
...
# Remove any riders that have been explicitly excluded
if !isempty(excluded_riders) && (:rider in propertynames(vgdata))
    before_exclusions = nrow(vgdata)
    vgdata = subset(vgdata, :rider => ByRow(r -> !(r in excluded_riders)))
    removed = before_exclusions - nrow(vgdata)
    if removed > 0
        println("🚫 Excluded $removed rider(s) via configuration")
    else
        println("ℹ️ No riders matched the exclusion list")
    end
end
...
if !isempty(racehash) && racehash != ""
    try
        pcsdata = getpcsraceranking(racestartlistquality; cache_config=race_cache)
        ...
        println("⚠️ PCS race data unavailable, using general rankings: $e")
...
if !isempty(excluded_riders) && (:rider in propertynames(complete_data))
    before_complete_exclusions = nrow(complete_data)
    complete_data = subset(complete_data, :rider => ByRow(r -> !(r in excluded_riders)))
    removed_complete = before_complete_exclusions - nrow(complete_data)
    if removed_complete > 0
        println("🚫 Removed $removed_complete excluded rider(s) from optimization dataset")
    end
end
...
if nrow(complete_data) >= 6
    ...
    if optimal_team !== nothing
        ...
        println("❌ No valid team found")
    end
else
    println("❌ Insufficient data for optimization")
end
```
...
if @isdefined(team_data) && nrow(team_data) > 0
    ...
    println("💰 Total Cost: $total_cost credits")
    println("🏆 Expected VG Points: $(round(total_vg_points)) points")
    println("💳 Budget Remaining: $(100 - total_cost) credits")
    println("👥 Team Size: $(nrow(team_data)) riders")
    println()
    ...
    println("⚠️ No optimal team generated - check configuration and try again")
end
```
    ...
    if @isdefined(complete_data)
        ...
        if nrow(not_chosen) > 0
            ...
            if nrow(cheap_options) > 0
                ...
                println("No riders ≤6 credits available")
            end
        else
